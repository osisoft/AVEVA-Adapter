...
| **Password**                    | Required for PI server endpoint  | `string`    | Basic authentication to the PI Web API OMF endpoint <br><br>_PI server:_<br>Allowed value: any string or a secret Id (see [Reference Secrets]<br>Default: `null`  |
...
| **ClientSecret**                | Required for OCS endpoint | `string`    | Authentication with the OCS OMF endpoint <br><br>Allowed value: any string or a secret Id (see [Reference Secrets]; can be null if the endpoint URL schema is `HTTP`<br>Default: `null`|
