...
| **FailoverTimeout** | Required | `datetime` | The failover timeout value of the failover group. This defines how frequently the adapter will send a heartbeat to the failover endpoint. Heartbeat will be sent every at FailoverTimeout / 2 interval <br><br>Allowed value: a string representation of date time using `hh:mm:ss` <br>|
